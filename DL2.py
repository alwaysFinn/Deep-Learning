'''
    1. Intelligence (지능)
        1) 학습 능력, 추론 능력, 지각 능력, 언어 이해 능력을 통칭
        2) 새로운 문제 발생시 그에 맞는 지시 제공
        3) 프로그램 : 문제를 해결할 때 따라야 하는 일들을 지시하는 것
        4) 컴퓨터가 스스로 방법을 찾아냄 : 데이터를 기반으로 스스로 학습할 수 있음
            - 알파고 : 컴퓨터에 바둑 경기의 규칙과 이전 경기의 많은 데이터를 알려줌
                    ==> 컴퓨터가 스스로 바둑의 원리 학습, 바둘을 둘 수 있었던 것
        5)                    파라미터 수정      ←-     평가
                                   ↓                   ↑
        입력    ==>   Model(동작을 결정하는 파라미터)  ==> 출력
                        융통성 있는 프로그램

        ◎ 이 과정이 반복되어 더 완벽한 결과물이 나옴
        ◎ 파라미터가 바뀌면 동작도 바뀜
        ◎ 좋은 동작이 나오도록 파라미터를 변경하는 일을 하는데, 이 과정을 Learning 즉, 학습이라고 부름

    2. Artificial Intelligence (인공 지능)
        1) 인간의 학습 능력과 추론, 지각 능력을 인공적으로 구현한 것
            - 인간처럼 사고, 인간처럼 행동, 합리적으로 사고, 합리적으로 행동
        2) 머신 러능(Machine Learning)은 매우 구체적이고 엄밀한 정의를 가지고 있음
        ...
        4)
                ...
                - 인공 신경망을 이용한 딥러닝 (Deap Learning) 분야 역시 이 머신러닝의 한 분야로 볼 수 있음

    3. 머신러닝 종류
        1) 지도학습 (Supervised Learning)
            - 데이터와 정답의 역할을 하는 레이블(Label)을 제공받음
            - 입력을 출력에 매핑하는 일반적인 규칙을 학습하는 것
            ex) 고양이, 개를 구분할 때 고양이인지 개인지 레이블링 된 데이터를 충분히 제공한 뒤 학습을 하도록 하는 과정 필요
                -> 학습단계 => 예측모델 => 테스트 단계
        2) 비지도학습 (Unsurpervised Learning)
