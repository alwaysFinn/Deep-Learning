    ...
    1. 인공 신경망
        ...
        2) perceptron
            ...
            - 단순 퍼셉트론은 선형 분류 문제만 해결한다는 한계가 있음
                ...
                - 퍼셉트론 하나로는 비선형 분류 문제를 해결 못함
                    -> 다층 퍼셉트론(multi-layer perceptron)을 만들면 해결

        3) 신경망
            - 입력층, 은닉층(중간층), 출력층

        4) 활성화 함수(activation function)
            - 입력값을 어떤 값으로 변환, 출력할지 결정하는 함수
            - 입력값과 가중치를 곱한 값들은 활성화 함수를 거쳐 출력갑싱 됨
            - 시그모이드 함수와 ReLU 함수
                - sigmoid function
                    - 입력값이 커질수록 출력값은 1에 가까움
                    - 입력값이 작아질수록 출력값은 0에 가까움
        ...
        5) 경사 하강법
            ...
            - 경사 하강법(gradient descent)
                ...
                - 절차
                    ① 현재 위치에서 기울기(경사) 구함
                    ② 기울기 아래 방향으로 일정 거리 이동
                    * 현재 위치의 기울기가 0이 될때까지 ①~② 반복
                        - 손실 함수가 최소가 될때까지
                - 학습률(learing rage)
        ...
        6) 순전파 & 역전파
            ...
            - 절차
                ...
                ② 손실값을 줄이는 방향으로 가중치의 기울기 구함
                ③ 기울기를 바탕으로 가중치를 갱신함 이때 출력층 -> 입력층 방향으로 가중치 갱신(역전파)
                ④ 갱신된 가중치를 바탕으로 반복. 손실값 감소
    ...
    2. 딥러닝 알고리즘 향상
        1) 드롭아웃(dropdout)
            ...
            - 얼마나 많은 뉴런을 드롭아웃할지는 하이퍼 파라미터로 설정할 수 있음
                - 드롭아웃 비율을 0.2로 설정하면 전체 뉴런의 20%를 제오히ㅏㅁ
            - 훈련 단계에서만 적용, 검증이나 예측 단계에서는 적용 X
        2) 옵티마이저(optimizer)
