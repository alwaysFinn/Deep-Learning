...
1. 퍼셉트론(perceptron)
    ...
    2) 다층 퍼셉트론
        - 은닉층이 있는 인공신경망
        - XOR 연산 : 은닉층이 최소 1개 이상 필요한 연산
    3) 딥러닝은 은닉층이 2개 이상인 인공신경망 모형의 학습하는 것
    4) 은닉층과 출력층
        - 각 입력값을 처리하는 합성함수와 출력으로 보내기 위한 활성화함수로 구성

2. 역전파(Backpropagation)
    1) 가중치와 편향을 해결하기 위한 방법으로 고안 (1974 ~ 1986)
        - 역전파가 XOR 문제뿐만 아니라 더 복잡한 문제도 해결할 수 있음이 증명됨 ==> 인공신경망 다시 부흥
    2) 신경망의 오차(예측값과 실제값의 차이)를 출력층에서 입력층으로 (거꾸로) 전파시켜 각 층(layer)의 가중치와 편향을 업데이트함
    3) 신경망이 깊어질수록 학습력이 더 좋아져야 하는데 기대하는 결과가 나오지 않는 일이 지속적 발생
...
# W = np.zeros()
W = np.zeros(len(X[0]))
#print(' 가중치를 저장하는 방법 : ' , W)

# 퍼셉트론 학습 알고리즘 구현
def perceptron_fix(X, Y, epochs=10):
    global W
    eta = 0.2       # 학습률
    ...
    for t in range(epochs):
        ...
        for i in range(len(X)):
            ...
            error = Y[i] - predict      # = 오차율(정답 - 예측값) / 10번의 학습을 거쳐 오차 변화 확인하기 위함
            W += eta * error * X[i]
            print('현재 처리 입력 = ' , X[i], ', 정답 = ', Y[i], ', 출력(뉴런 활성화 여부) = ', predict, ', 변경된 가중치 = ', W)
        print('-------------------------------------------------------------------------------------')

perceptron_fix(X, y, 10)