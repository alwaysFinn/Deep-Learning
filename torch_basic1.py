'''
    1. 파이토치(Pytorch) 개요
        ...
        2) GPU에서 텐서 조작 및 동적 신경망 구축이 가능한 프레임워크
            - 딥러닝에서는 기울기 계산할 때 미분 사용, GPU를 사용하면 빠른 계산 가능
    ...
    3. 벡터
        ...
        텐서
            - 3차원 이상의 배열 형태
                ...
                - 3차원의 축(채널) : axis = 2 (텐서)
            - torch.tensor()
            - tensor간의 연산을 수행할 때, 기본적으로 두 테서는 같은 장치에 있어야 함(CPU / GPU)
                - 서로 다른 장치ㅓ(device)에 있는 텐서끼리

    4. 파이토치 특징
        1) 신경망은 연산 그래프를 이용하여 계산을 수행
        2) 네트워크 학습될때 손실함수의 기울기가 가중치와 바이어스(절편)를 기반으로 계산되며
            이후 경사 하강법을 사용하여 가중치가 업데이트
        3) 이때 연산 그래프를 이용하여 이 과정이 효과적으로 수행
        4) 파이썬 환경과 쉽게 통합될 수 있음
        5) 모델 훈련을 위한 CPU 사용률이 텐서플로와 비교하여 맞음
        6) 학습 및 추론 속도가 빠르고 다루기 쉬움

    5. 파이토치 아키텍쳐
        1) 파이토치 API 계층 : 사용자의 편의성 여러 패키지들 제공
            - torch : GPU를 지원하는 텐서 패키지
                      CPU뿐만 아닌 GPU에서도 연산 가능 ( 빠른 속도로 많은 양의 계산 가능 )
                      파이토치에서는 텐서가 핵심
            - torch.autograd : 자동 미분 패키지
            - torch.nn : 신경망 구축 및 훈련 패키지
        2) 파이토치 엔진 (라이브러리)
            -  C++로 감싼 래핑(wrapping)한 이후 Python API 형태로 제공
